...
/* Variable length arrays are supported by C99. As of 2015, they are
   ...
   of your program may be undefined. Setting a variable length array
   to a length of 0 or a negative number may also be undefined. For
...
{
	char vla[n]; // the compiler doesn't know how long VLA is until run-time. Therefore, this is a variable length array (VLA).
