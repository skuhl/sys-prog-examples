...
int main(int argc, char *argv[])
{
	...
	printf("This program only prints some of the information about the file that stat() provides.\n");


	/* Use chmod() to remove the execute permission bit from this
	 * executable. */

	// Turn off the user execute bit.
	fileStat.st_mode = fileStat.st_mode & ~S_IXUSR;
	chmod(file, fileStat.st_mode);
	printf("\n");
	printf("We used chmod() to remove the executable bit from this file. Run 'chmod a+x %s' to restore it.\n", argv[0]);
